...
struct CollectingNodeData
{
    ...
    // SCP stats
    medida::Timer mSCPFirstToSelfLatencyTimer;
    medida::Timer mSCPSelfToOtherLatencyTimer;
...
class SurveyDataManager : public NonMovableOrCopyable
{
  public:
    ...
    bool nonceIsReporting(uint32_t nonce);
  ...
  private:
    ...
    // Reports on the current survey phase. May modify internal structures if
    // data has expired since the last call to `phase`.
    SurveyPhase phase();
    ...
    // Given a peer, return the map of peer data that corresponds to that peer's
    // role.
    std::unordered_map<NodeID, CollectingPeerData>&
    getPeerDataMapFromPeer(Peer const& peer);
