...
1. **Cross-node invariants** — properties that require observing multiple nodes simultaneously (safety, cross-node liveness, skip-ancestry consistency).
...
The original proposal in `testing.md` described "Protocol MBT" as either (a) a set of high-level invariants over inter-node interactions, or (b) a model object that predicts the correct outcome ("skip, confirm original value, etc.") given a scenario.
...
- **Validity of non-skip externalization**: if the network externalizes a non-skip value for slot `N`, at least one honest node obtained and validated the pre-image of that value before externalization.
- **Timing-conditional liveness**: if at any point during slot `N` a quorum of honest nodes held a valid tx set before their respective skip timers expired, the network must externalize that tx set (i.e. must not externalize skip).
- **Skip-ancestry consistency**: if the network externalizes skip for slot `N`, then for slot `N+1`, `previousLedgerHash` / `previousLedgerVersion` on all honest-node ballots are consistent with slot `N` being treated as empty.
- **Quorum convergence on `combineCandidates`**: after a bounded number of ballot rounds in which a quorum has at least one valid tx set, the network converges to externalizing a valid value or skip — not perpetual churn.
- **No honest-node divergence under good timing**: under scenarios where every honest node has the tx set before any skip timer fires, every honest node externalizes the same non-skip value.
...
- **Network skip rate**: under "normal timing" scenarios, `skipped_ledgers / total_ledgers < 1/1000` across the network.
...
- **Good-timing happy path**: under scenarios where the tx set arrives at every node before its first ballot timer, the network skip count is zero. This is the closest we get to an oracle check without writing one.
...
- **Malicious-node profiles (reused from component-level)**: scaled up to multiple attackers. "One malicious withholds" vs. "two collude on conflicting skip values" vs. "minority sends invalid tx sets."
- **Per-node skip-timer calibration**: heterogeneous timeout values to stress the dynamic-timeout mechanism.
...
- Tx set arrives for a quorum but the remainder's skip timer fires first (tests the "fast quorum, slow minority" path).
- Skip timer on one node fires mid-`combineCandidates` on another.
